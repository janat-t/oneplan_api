...
Plan_detail.removePlanIdOne = (id, day, order, result) => {
  sql.query(
    "DELETE FROM plan_detail WHERE plan_id = ? AND day = ? AND attraction_order = ?",
    [id, day, order],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Plan_detail.updateByIdOne = (id, day, order, plan_detail, result) => {
  sql.query(
    "UPDATE plan_detail SET plan_id = ?, day = ?, attraction_order = ?, start_time = ?, end_time = ?, time_spend = ?, attraction_id = ?, description = ? WHERE plan_id = ? AND day = ? AND attraction_order = ?",
    [
      plan_detail.plan_id,
      plan_detail.day,
