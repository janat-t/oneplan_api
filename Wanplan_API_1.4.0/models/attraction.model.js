...
Attraction.updateById = (id, attraction, result) => {
  sql.query(
    "UPDATE attraction SET attraction_name = ?, attraction_name_thai = ?, attraction_type = ?, open_time = ?, close_time = ?, description = ?, ward_id = ? WHERE attraction_id = ?",
    [attraction.attraction_name, attraction.attraction_name_thai, attraction.attraction_type, attraction.open_time, attraction.close_time, attraction.description, attraction.ward_id, id],
	(err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
