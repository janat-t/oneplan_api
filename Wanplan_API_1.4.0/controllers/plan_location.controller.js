...
exports.create = (req, res) => {
  ...
  Plan_location.create(plan_location, (err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating the plan_location."
      });
    else res.send(data);
  });
};
      ...
      if (err.kind === "not_found") {
		...
		for(let i=0;i<data.length;i++){
			...
			Plan_location.create(plan_location, (err, data2) => {
				...
				res.status(500).send({
					message:
					err.message || "Some error occurred while duplicating plan_location with plan_id." + req.params.planId
				});
...
exports.deletePlanId = (req, res) => {
  Plan_location.removePlanId(req.params.planId, (err, data) => {
    ...
    } else res.send({ message: `Plan_location with plan_id ${req.params.planId} was deleted successfully!` });
  });
};
...
exports.deleteCityId = (req, res) => {
  Plan_location.removeCityId(req.params.cityId, (err, data) => {
    ...
    } else res.send({ message: `Plan_location with city_id ${req.params.cityId} was deleted successfully!` });
  });
};
