...
const Image = function(imagex) {
  ...
  this.attraction_id = imagex.attraction_id;
};
...
Image.findByAttractionId = (id, result) => {
  sql.query(`SELECT * FROM image WHERE attraction_id = ${id}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Image.removeAttractionId = (id, result) => {
  sql.query("DELETE FROM image WHERE attraction_id = ?", id, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
    ...
    console.log("deleted image with attraction_id: ", id);
    result(null, res);
  });
};
...
Image.updateById = (id, imagex, result) => {
  sql.query(
    "UPDATE image SET image_id = ?, image_name = ?, image_url = ?, attraction_id = ? WHERE image_id = ?",
    [imagex.image_id, imagex.image_name, imagex.image_url, imagex.attraction_id, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
